 ...
 *      ]
 *
 * The "useNamedArgs" argument controls if all the arguments
 * should be named:
 *
 ...
 * @param {boolean} useNamedArgs Should the arguments be in the named format?
 * @return {Directive[]}
 */
export function parseDirectives(content, useNamedArgs = false) {
    const directives = [];
    ...
    for (var i = 0; i < content.length; i++) {
        ...
        switch(state) {
            case 'action':
                ...
                throw new Error(`Unexpected character ${char} after action "${currentActionName}" in directive ${content}`);
